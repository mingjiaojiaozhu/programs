...
void enqueueHead(ArrayNode *arrays, int value) {
    ...
    arrays->head = arrays->head ? arrays->head - 1 : arrays->capacity - 1;
    arrays->values[arrays->head] = value;
}

void enqueueTail(ArrayNode *arrays, int value) {
    if ((arrays->tail + 1) % arrays->capacity == arrays->head) {
        int *aux = (int *) malloc(sizeof(int) * (arrays->capacity + 16));
        if (arrays->head < arrays->tail) {
            memcpy(aux, arrays->values + arrays->head, sizeof(int) * (arrays->tail - arrays->head));
...
int dequeueHead(ArrayNode *arrays) {
    if (arrays->head != arrays->tail) {
...
int dequeueTail(ArrayNode *arrays) {
    if (arrays->head != arrays->tail) {
        arrays->tail = arrays->tail ? arrays->tail - 1 : arrays->capacity - 1;
        return arrays->values[arrays->tail];
    }
    return 1 << 31;
}
